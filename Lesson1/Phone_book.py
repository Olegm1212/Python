...
def copy(file_name: str):
    ...
    with open(file_name, 'r',encoding='utf-8') as f:
        ...
        print(list(zip(range(1,len(data)+1),data)))
        while True:
            id = input("Введите id нужного пользователя для копирования в другой файл: ")
            if 0<int(id)<len(data)+1:
                break
            print("не верный id: ") 
        new_file = input("Введите название файла куда копировать: ") 
        with open(new_file, 'a', encoding='utf-8') as fd:
            fd.write(data[int(id)-1])
...
def main():
    ...
    while not flag_exit:
        ...
        print('6 - копирование записи по id')
